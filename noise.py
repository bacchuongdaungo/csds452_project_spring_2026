...
def gaussian_noise_test():
    for i in range(6):
        ...
        # output_path = noisy_root / "gaussianSTD_test" / f"ihdp_npci_1_test_noisy_std_{std_label}.csv"
...
def drop_test():
    ...
    for _ in range(5):
        ...
        reduced_data = np.concatenate(
            [data_drop[:, :5], data_drop[:, 5:][:, keep_x_columns]],
            axis=1,
        )
        ...
        # output_path = noisy_root / "drop_3_rep" / f"ihdp_npci_1_test_drop_{total_drop_columns}.csv"
        write_csv(reduced_data, output_path, csv_columns=reduced_columns)

#Both noise and drop
def both_noise_test():
    data_noisy_drop = data.copy()
    data_noisy_drop[:, 5:] = apply_gaussian_noise(
            data_noisy_drop[:, 5:],
            gaussian_std=0.5,
            gaussian_mean=0.0,
            rng=np.random.default_rng(42),
    ...
    for _ in range(5):
        ...
        reduced_data = np.concatenate(
            [data_noisy_drop[:, :5], data_noisy_drop[:, 5:][:, keep_x_columns]],
            axis=1,
        )
        reduced_columns = [*base_columns, *[feature_columns[index] for index in keep_x_columns]]
        output_path = noisy_root / "both_noise" / f"ihdp_npci_1_noisy_drop_{total_drop_columns}.csv"
        # output_path = noisy_root / "both_noise" / f"ihdp_npci_1_test_noisy_drop_{total_drop_columns}.csv"
        write_csv(reduced_data, output_path, csv_columns=reduced_columns)
